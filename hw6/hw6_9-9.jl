...
# ╔═╡ 4904f2f1-54f7-4fcb-b8ed-328660f2ae4d
using CairoMakie, DifferentialEquations, LsqFit

# ╔═╡ 04ef2f3a-93a3-11ec-01d2-eb4c2872e560
md"## Problem 9-9"

# ╔═╡ 867b5292-dc28-4e13-afba-e16d3c38b256
import Plots

# ╔═╡ 0314e348-f872-44d8-8d9f-2a9c0c9048ab
md"
Hydrogen peroxide is catalytically decomposed by catalase.
"

# ╔═╡ f7408ae8-b8e5-4223-bbeb-0841b55bad8a
t = [0, 10, 20, 50, 100] # time, minutes

# ╔═╡ 7784cfc2-35d9-4c1d-af8f-8c8f0033909e
S = [0.02, 0.01775, 0.0158, 0.0106, 0.005] # peroxide concentration, mol/L

# ╔═╡ 875ddce3-fcdc-4f9f-8692-add10e08d229
md"**(a)** Determine the Micaelis-Menten parameters $V_{max}$ and $K_M$"

# ╔═╡ 61d42ac5-daa9-4886-b8c0-60318c19043b
md"Using the initial rate method to estimate the MM constants:

$v_n \approx \frac{S_n-S_{n+1}}{t_{n+1}-t_n}$
"

# ╔═╡ 91abdb68-a124-4778-bc66-3ce0b80ddb1e
v_approx = [(S[n] - S[n+1]) / (t[n+1] - t[n]) for n in 1:length(t)-1]

# ╔═╡ aa3fdb98-e9c5-4998-862e-3fa85c0f03cc
md"
$v_1 = \frac{V_{max}S_1}{K_M+S_1}$

$v_2 = \frac{V_{max}S_2}{K_M+S_2}$

$(K_M+S_2)v_2 = (K_M+S_1)v_1$

$K_M=\frac{S_1v_1-S_2v_2}{v_2-v_1}$

$V_{max} = \frac{(K_M+S_1)v_1}{S_1}$
"

# ╔═╡ 6ca31f07-5738-4cfd-a688-b69832a88ec3
Km = (S[1] * v_approx[1] - S[2] * v_approx[2]) / (v_approx[1] - v_approx[2])

# ╔═╡ 6d6d7ad5-1257-4650-a86d-eb9f801025ca
Vmax = (Km + S[1]) * v_approx[1] / S[1]

# ╔═╡ 7ef48119-284f-4f4c-af02-9c806635b5c5
md"**(b)** At 3X the catalase concentration, what is $S(20)$?"

# ╔═╡ c49d2d3e-09f7-428a-913e-3aea8353d221
md"
Tripling the enzyme concentration causes $V_{max}$ to triple as well:

$V_{max}^\prime = 0.00183 \frac{\text{M}}{\text{min}}$

$-\frac{dS}{dt} = \frac{V_{max}^\prime S}{K_M+S}$

Solving the ODE:
"

# ╔═╡ 8e1b35f1-97ca-48b0-b19c-30bea32845f5
begin
	dS_dt(S, p, t) = -3 * Vmax * S / (Km + S)
	tspan = (0., 30.)
	prob = ODEProblem(dS_dt, S[1], tspan)
	sol = solve(prob)
end;

# ╔═╡ 0fa044d5-2c7d-4340-9e86-91991f27858c
Plots.plot(sol)

# ╔═╡ 0e8c9502-02ec-4ad3-82c8-cfab8156827f
md" $S(20) =$ $(round(sol(20), digits=4)) M"
