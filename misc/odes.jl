...
# ╔═╡ 54021392-7e10-11ec-15a3-ed187a92abd3
using DifferentialEquations, CairoMakie

# ╔═╡ 8fda3be7-1aed-4d4b-a789-f7b0205a0681
md"# Coupled ODE Solver"

# ╔═╡ c16d1e85-44b8-4ab3-9240-a77cc3a36dbf
solve_ode(args...) = solve(ODEProblem(args...)); # shortcut syntax

# ╔═╡ 23f7f95e-2513-479d-b2da-8e225d673dde
md"## System of Equations"

# ╔═╡ 94785c8b-7ceb-4dad-a8f6-a63a5e429aca
md"## Parameters"

# ╔═╡ a70b5dc0-dbe7-4dcb-b542-5ac355839240
begin
	# constants
	const Fₐ₀ = 5
	const ΔHᵣₓₙ = -1e5
	...
	# initial state
	u₀ = Float64[0, 100, 50] # X, T, T₀

	# independent variable interval
	V = (0, 20)
end;

# ╔═╡ 993213d8-12e0-4d0d-9249-9030173aa5ae
begin
	# rate law
	rₐ(T, X) = (1 - X) * T
	...
	# reactor temperature
	dT_dV(T, X) = (rₐ(T, X) * ΔHᵣₓₙ - UA * (T - T₀)) / (Fₐ₀ * cₚ)

	# co-current heat exchanger temperature
	dT₀_dV(T) = UA * (T - T₀) / (ṁ₀ * cₚ₀)
end;

# ╔═╡ 565762b7-6707-44f6-a9bd-6359bfdca14e
md"## Solution"

# ╔═╡ b28c7037-6999-4849-945d-f5d96d1ac978
begin
	local ode_system = (f, u, _, t) -> begin
		X, T, T₀ = u
		f[:] = [dX_dV(T, X), dT_dV(T, X), dT₀_dV(T)]
	end
	
	local sol = solve_ode(ode_system, u₀, V)

	local fig = Figure()
	local ax1 = Axis(fig[1, 1])
	lines!(ax1, sol.t, [u[1] for u in sol.u])
	fig
end
