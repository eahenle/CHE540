...
# ╔═╡ 646467f1-bc4b-4d8c-8ce7-1390d4cb1de5
using DifferentialEquations, DataFrames, CairoMakie, ColorSchemes, NLsolve

# ╔═╡ cd4616a9-97cf-46c8-a046-5b149ac5ef67
set_theme!(theme_light()); update_theme!(fontsize=20, linewidth=4)

# ╔═╡ 2af462c0-7b3b-11ec-3ee5-0d9ca3559537
md"

## a system of ODEs encountered in chemical reaction engineering

an exothermic reaction proceeds along a tubular reactor. a jacket surrounds the reactor, in which cooling water flows co-currently with the reacting mixture. heat transfer occurs from the reacting mixture to the cooling water. the process is at steady state conditions. 

for convenience, we represent the spatial location along the tubular reactor (and heat exchanger) with the volume $V$ of the reactor from the inlet to that point. the reactor is of total volume $V_t$.

**the state variables**, functions of the location along the tubular reactor, indicated by $V$, are:
*  $X=X(V)$: conversion of the reactant in the reactor
*  $T=T(V)$: temperature in the reactor
*  $T_o=T_o(V)$: temperature of the cooling water

the following **system of coupled, ordinary differential equations** governs the evolution of the state variables along the tubular reactor:
```math
\begin{align}
	F_{a0} \frac{dX}{dV} &= -r_a & \text{\color{gray} mass balance on PF reactor} \\
F_{a0} c_p \frac{dT}{dV} &= r_a \Delta H_{rxn} -U A(T-T_o)& \text{\color{gray} energy balance on PF reactor} \\
  \dot{m}_o c_{p,o} \frac{d T_o}{dV} &= U A (T-T_o) & \text{\color{gray} energy balance on heat xchanger}\\
\end{align}
```

**the parameters** are:
*  $F_{a,0}$: inlet flow rate of the reactant
*  $r_a$: rate of reaction
*  $\Delta H_{rxn}$: enthalpy of reaction
*  $U$: heat transfer coefficient
*  $A$: area of contact between reactor walls and cooling water
*  $c_{p}$: specific heat capacity of the reacting mixture
*  $\dot{m}_o$: mass flow rate of cooling water
*  $c_{p,o}$: specific heat capacity of the cooling water

n.b. $r_a=r_a(X, T)$.
"

# ╔═╡ 28a66186-8148-402f-88c0-0b3b370d83cb
md"
!!! note \"how do I learn more?\"
	* basics of `Julia`? learning resources [here](https://julialang.org/learning/)
	* `Pluto.jl` computational notebooks? YouTube videos linked [here](https://github.com/fonsp/Pluto.jl)
	* `DifferentialEquations.jl` for solving ODEs? docs [here](https://diffeq.sciml.ai/stable/)
	* `CairoMakie.jl` for data viz? docs [here](https://makie.juliaplots.org/stable/)
	* `NLsolve.jl` for solving nonlinear systems of algebraic equations? docs [here](https://github.com/JuliaNLSolvers/NLsolve.jl).

for more on Julia, see MIT's Intro to Computational Thinking course [here](https://computationalthinking.mit.edu/Spring21/) and \"Julia: come for the syntax and stay for the speed\" [here](https://www.nature.com/articles/d41586-019-02310-3). 
"

# ╔═╡ d4efc1ab-c901-47c1-9bf0-0fe47c67b6c1
begin
	#= 
		parameters
	=#
	...
	Fₐ₀ = 14.67        # kmol / hr
	cₐ₀ = 1.86         # mol / m³ 
	ΔHᵣₓₙ  = -34500.0  # kJ / kmol
	Vₜ = 5.0           # m³
	cₚ = 159.0         # kJ / (kmol - K)

	# reaction modeling
	k(T)  = 31.1 * exp(7906.0 * (T - 360.0) / (360.0 * T))         # hr⁻¹
	K(T) = 3.03 * exp(ΔHᵣₓₙ / 8.314 * (T - 333.0) / (333.0 * T))  # unitless
	rₐ(T, X) = -k(T) * cₐ₀ * (1 - (1 + 1 / K(T)) * X) # kmol / (hr - m³)
...
# ╔═╡ da23b358-8721-4737-a58f-729d8156ecf4
#= view the ODE system as:
    du / dV = f(u)
with u = u(V) := [X(V), T(V), Tₒ(V)]
=#
function update_f!(f, u, _, t)
	# unpack state vector
	X, T, Tₒ = u
	...
	f[1] = -rₐ(T, X) / Fₐ₀
	# dT/dV  = f[2]
	f[2] = (rₐ(T, X) * ΔHᵣₓₙ - UA * (T - Tₒ)) / (Fₐ₀ * cₚ)
	# dTₒ/dV = f[3]
	f[3] = UA * (T - Tₒ) / (ṁₒ * cₚₒ)
end

# ╔═╡ da1cfe30-78ae-453d-9c26-c7eb9a72b748
# [X(0), T(0), Tₒ(0)]
u₀ = [X₀, T₀, Tₒ₀]

# ╔═╡ 779c9b2f-9659-482d-a79f-13abb92ad7b8
begin
	V_span = (0.0, Vₜ)
	prob = ODEProblem(update_f!, u₀, V_span, 
		              saveat=0.01)
	data = DataFrame(solve(prob))
	rename!(data, Dict("timestamp" => "V",
		               "value1" => "X",
		               "value2" => "T",
		               "value3" => "Tₒ")
	)
...
# ╔═╡ 301fdd20-4d31-4ffe-a7e9-b3c82e33f604
begin
	fig = Figure()
	...
	axs[1].ylabel = "X(V)"
	axs[1].title = "conversion"
	lines!(axs[1], data[:, "V"], data[:, "X"], color=ColorSchemes.Accent_3[1])
	ylims!(axs[1], 0, 1)
	
	axs[2].ylabel = "T(V)/Tₒ(V) [K]"
	axs[2].title = "temperature"
	lines!(axs[2], data[:, "V"], data[:, "T"], 
		color=ColorSchemes.Accent_3[2], label="T(V)")
	lines!(axs[2], data[:, "V"], data[:, "Tₒ"], 
		color=ColorSchemes.Accent_3[3], label="Tₒ(V)", linestyle=:dash)
	axislegend(axs[2])
...
# ╔═╡ fafd0863-0874-425f-893a-0c3c4885c32a
md"at what $V$ is the peak temperature?"

# ╔═╡ f82473f6-9f31-48a3-9cf2-213c49d6341d
data[argmax(data[:, "T"]), "V"]

# ╔═╡ fd66135d-498c-4cf7-bf72-8bd3cdf28a43
md"what is the steady-state temperature and conversion?
"

# ╔═╡ ac010e12-0494-4f02-8ca6-276209cebb6d
rhs!(f, u) = update_f!(f, u, NaN, NaN) # pass in bogus numbers for parameter vector and time.

# ╔═╡ f86f1ae0-728a-47ce-b053-79b374f5c508
nlsolve(rhs!, [0.71, 336.0, 335.0])
...
git-tree-sha1 = "2228e0d2252d0bf385377a0d20130166d033277e"
